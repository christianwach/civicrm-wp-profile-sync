...
class CiviCRM_Profile_Sync_ACF_ACFE_Form_Address_Country extends acf_field {
	...
	public function __construct( $parent ) {
		...
		// Define label.
		$this->label = __( 'CiviCRM Country', 'civicrm-wp-profile-sync' );
	...
	public function get_country( $value, $post_id, $field ) {
		...
		if ( $contact_id !== false ) {
			...
			// Init Country/Province ID.
			$country_id = false;

			// Does this Field sync with the Primary Address?
			if ( ! empty( $field['country_is_primary'] ) ) {

				// Assign Country/Province ID from the Primary Address.
				foreach( $addresses AS $address ) {
					if ( ! empty( $address->is_primary ) ) {
						$country_id = (int) $address->country_province_id;
						break;
					}
				}
			...
			// We need a Location Type.
				...
				// Assign Country/Province ID from the type of Address.
				foreach( $addresses AS $address ) {
			...
			// Overwrite if we get a value.
			if ( $country_id !== false ) {
				$country = $this->civicrm->address->country_province_get_by_id( $country_id );
				$value = $country['name'];
			}
