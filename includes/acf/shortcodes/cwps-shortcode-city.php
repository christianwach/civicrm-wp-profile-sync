<?php
/**
 * CiviCRM Profile Sync City Shortcode Class.
 *
 ...
 * @since 0.4
 */
class CiviCRM_Profile_Sync_ACF_Shortcode_City {
	...
	public function shortcode_render( $attr, $content = '', $tag = '' ) {
		...
		if ( is_feed() ) {
			return '<p>' . __( 'Visit the website to see the City.', 'civicrm-wp-profile-sync' ) . '</p>';
		}
		...
		$content = cacf_get_city_by_type_id( $atts['field'], $atts['location_type'], $atts['style'], $atts['post_id'] );
	...
	public function shortcake() {
		...
		*/
		...
		$settings = [
			...
			// Window title.
			'label'         => esc_html__( 'CiviCRM City', 'civicrm-wp-profile-sync' ),
	...
	public function shortcake_styles( $mce_css ) {
		...
		$mce_css .= ', ' . CIVICRM_WP_PROFILE_SYNC_PATH . 'assets/css/cwps-shortcode-city.css';
